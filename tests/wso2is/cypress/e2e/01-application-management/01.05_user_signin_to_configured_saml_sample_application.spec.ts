...
import {
    ...
    CypressKeywords, HTMLKeyWordsConstants, SAMLKeywords
} from "../../constants";
import { ApplicationProtocol, RequestContentTypes } from "../../models";
import {
    ApplicationCreatePage,
    ApplicationEditPage,
    ApplicationProtocolEditPage,
    ApplicationsMainPage, AsgardeoSampleAppLoginPage,
    ConsoleCommonPageElements,
...
const sampleAppRedirectUrlOrigin = TestE2EConfigUtils.javaSampleAppDefaultOriginUrl,
    ...
    duplicateApplicationIssuer = "https://www.wso21.com",
    multipleAccessCustomUser = {
        password: EnvironmentConfig.getUserCredentials().password,
        userName: "multipleAccessUser@test.com"
    };

let testId: string, samlWebAppPayload: Cypress.ObjectLike, userCreationPayload: Cypress.ObjectLike,
    multipleAccessUserPayload: Cypress.ObjectLike;
 ...
 */
describe("01.05 - Sign-in to Configured SAML Sample Application", () => {
    ...
    before(() => {
        ...
        //Prerequisite : Multiple Access User Creation
        //This user is created to enable consent for multiple ACS SAML Response Validation
        cy.fixture(TestE2EConfig.scimUserCreationPayload).then(($userPayload) => {

            // Store content of the fixture in a global variable.
            multipleAccessUserPayload = $userPayload;

            multipleAccessUserPayload.userName = ProductConfig.userStore + "/" + multipleAccessCustomUser.userName;
            multipleAccessUserPayload.password = multipleAccessCustomUser.password;
            multipleAccessUserPayload.emails[0].value = multipleAccessCustomUser.userName;

            // Create user via SCIM API.
            //Native function is used as filter differs from existing custom command
            cy.createSCIMUser(TestE2EConfigUtils.getTenantedPath(EnvironmentConfig.getTenant())
                , EnvironmentConfig.getTenantAdminCredentials(), multipleAccessUserPayload);
        });
    });

    qase(306,
        it("01.05.01 - Verify the tenant admin can create a SAML Based Traditional" +
    ...
    qase(315,
        it("01.05.02 - Attempt creating an application without mandatory details", () => {
            ...
            ConsoleCommonPageElements.getCommonModalHeader()
                .should(CypressKeywords.ASSERTION_TO_CONTAIN, ApplicationCreatePageConstants.WEB_APPLICATION_HEADER);
            cy.selectApplicationProtocol(ApplicationProtocol.SAML);
            ApplicationCreatePage.getApplicationRegisterBtn().click();

            ApplicationsMainPage.getErrorPrompt().should(CypressKeywords.ASSERTION_TO_CONTAIN,
                ApplicationCreatePageConstants.APPLICATION_MANDATORY_MESSAGE);
        })
    );

    qase(314,
        it("01.05.03 - Attempt creating an application with existing issuer", () => {
            ...
            ConsoleCommonPageElements.getCommonModalHeader()
                .should(CypressKeywords.ASSERTION_TO_CONTAIN, ApplicationCreatePageConstants.WEB_APPLICATION_HEADER);
            cy.selectApplicationProtocol(ApplicationProtocol.SAML);
            ApplicationCreatePage.getAppName().type(samlWebAppPayload.generalConfigs.appName + "_1");
            ApplicationCreatePage.getSAMLWebAppIssuer().within(() => {

                cy.get(HTMLKeyWordsConstants.INPUT_FIELD).type(samlWebAppPayload.generalConfigs.issuer);
            });
            ApplicationCreatePage.getSAMLWebAccessURLs().within(() => {

                cy.get(HTMLKeyWordsConstants.INPUT_FIELD).type(samlWebAppPayload.generalConfigs.redirectUri);
            });
            ApplicationCreatePage.getApplicationRegisterBtn().click();

            ApplicationsMainPage.getErrorPrompt().should(CypressKeywords.ASSERTION_TO_CONTAIN,
                ApplicationCreatePageConstants.DUPLICATE_ISSUER_MESSAGE);
        })
    );

    qase(317,
        it("01.05.04 - Attempt creating an application with existing application name", () => {
            ...
            ConsoleCommonPageElements.getCommonModalHeader()
                .should(CypressKeywords.ASSERTION_TO_CONTAIN, ApplicationCreatePageConstants.WEB_APPLICATION_HEADER);
            cy.selectApplicationProtocol(ApplicationProtocol.SAML);
            ApplicationCreatePage.getAppName().type(samlWebAppPayload.generalConfigs.appName);
            ApplicationCreatePage.getSAMLWebAppIssuer().within(() => {

                cy.get(HTMLKeyWordsConstants.INPUT_FIELD).type(duplicateApplicationIssuer);
            });
            ApplicationCreatePage.getSAMLWebAccessURLs().within(() => {
            ...
            ApplicationsMainPage.getErrorPrompt().should(CypressKeywords.ASSERTION_TO_CONTAIN,
                ApplicationCreatePageConstants.DUPLICATE_APP_NAME_MESSAGE);
        })
    );

    it("01.05.05 - Verify whether the SAML sample app authentication configs " +
    ...
    qase(334,
        it("01.05.06 - Verify able to add mandatory attributes for a SAML based Web Application. ", () => {
    ...
    it("01.05.07 - Verify successfully startup the tomcat server.", () => {
    ...
    qase(318,
        it("01.05.08 - Verify whether the user is able to try out the saml sample application successfully.",
            () => {
                cy.session(EnvironmentConfig.getUserCredentials().userName, () => {
                    ...
                    // Stop the Tomcat server to update the next set of tests
                    cy.task(CypressTaskConfig.taskKillPID, tomcatServerPort);
                    cy.task(CypressTaskConfig.taskKillPID, tomcatServerShutDownPort);
                });
            })
    ...
    qase(316,
        it("01.05.09 - Verify configure multiple Assertion Consumer URLs successfully", () => {

            //Update the protocol with multiple URLs
            updateProtocolConfig();

            //Replace Assertion Consumer URL with home1.jsp
            cy.readFile(TestE2EConfig.javaSampleAppTomcatLocation +
                TestE2EConfig.javaSAMLSampleAppConfigFileLocation).then(configuration => {
                    ...
                    const updatedConfig = (configuration + "")
                        .replace("SAML2.AssertionConsumerURL=" + samlWebAppPayload.generalConfigs.redirectUri,
                            "SAML2.AssertionConsumerURL=" + samlWebAppPayload.generalConfigs.redirectUri2);

                    cy.writeFile(TestE2EConfig.javaSampleAppTomcatLocation +
                        TestE2EConfig.javaSAMLSampleAppConfigFileLocation, updatedConfig).then(() => {

                            //Restart the tomcat server with the new sample property configurations
                            cy.task(CypressTaskConfig.taskStartUpTomcatServer, TestE2EConfig.tomcatServerLocation);
                            HostControls.waitForUrl(TestE2EConfigUtils.javaSampleAppDefaultOriginUrl +
                                TestE2EConfig.samlJavaSampleAppAccessUrl);
                        });
    ...
    qase(335,
        it("01.05.10 - Verify login with multiple Assertion Consumer Service URL successfully",
            () => {
                ...
                SampleApplicationPage.getSampleLoginButton().click();
                cy.log("User: " + multipleAccessCustomUser.userName +
                    loginToTheApplication);
                cy.provideSampleAppLocalCredentials(multipleAccessCustomUser);
                ...
                cy.wait("@getSAMLPostRequest1").then(({ request }) => {
                    const parser = new DOMParser();
                    ...
                    const samlResponseDom = parser.parseFromString(decodedResponse.toString(),
                        RequestContentTypes.XML);
                    cy.log("Parsed SAML Response :" + samlResponseDom);
                    validateAssertionConsumerURL(samlResponseDom, samlWebAppPayload.generalConfigs.redirectUri2);
                    validateIssuer(samlResponseDom, samlWebAppPayload.generalConfigs.issuer);
                    validateBusinessUserEmail(samlResponseDom, multipleAccessCustomUser.userName);
                    validateBusinessUserGivenName(samlResponseDom, userCreationPayload.name.givenName);
                });
            })
    );
    ...
    after(() => {
        ...
        cy.deleteSCIMUser(TestE2EConfigUtils.getTenantedPath(EnvironmentConfig.getTenant())
            , EnvironmentConfig.getTenantAdminCredentials(),
            false, multipleAccessCustomUser.userName);
    ...
    /**
     * This function is used to update the protocol configuration of an application
     */
    function updateProtocolConfig() {
        ...
        ApplicationsMainPage.getInputSearchByApplicationName().type(samlWebAppPayload.generalConfigs.appName)
            .trigger(CypressKeywords.TRIGGER_KEYDOWN, CypressKeywords.TRIGGER_KEY_ENTER);
        ApplicationsMainPage.getBtnRowOfApplicationsTable(samlWebAppPayload.generalConfigs.appName).within(() => {
        ...
        ApplicationProtocolEditPage.getApplicationConsumerServiceURLInput().find(HTMLKeyWordsConstants.INPUT_FIELD)
            .then(element => {
                cy.wrap(element)
                    .type(samlWebAppPayload.generalConfigs.redirectUri2 + CypressKeywords.PRESS_ENTER_KEY);
            });

        ApplicationProtocolEditPage.getProtocolUpdateBtn().click();
